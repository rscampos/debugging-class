 ...
 * source09.c - Challenge from the book "The art of debugging with 
 * gdb, ddd and eclipse" - Chapter 4. 
...
char Chomp(CString *cstring){
	char lastchar = *(cstring->str + cstring->len); // Shorten the string by one
...
CString *Append_Chars_To_CString(CString *p, char *str) {
	...
	// Free old string and make CString point to the new string free(p->str);
	p->str = newstr;
	return p;
}
